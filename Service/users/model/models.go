...
func InitDb() (*gorm.DB, error) {
	...
	db, err := gorm.Open("mysql",
		"root:199791@tcp(127.0.0.1:3306)/search_house?parseTime=True&loc=Local")
	...
	if err == nil {
		...
		// 设置 连接池属性 最大空闲连接数， 创建之后连接最长存活时间  最大同时打开的连接数(使用中+空闲)
		GlobalConn.DB().SetMaxIdleConns(10)
		GlobalConn.DB().SetConnMaxLifetime(100)
		GlobalConn.DB().SetMaxOpenConns(20)
